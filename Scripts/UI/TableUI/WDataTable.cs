...
namespace WDT
{
    ...
    public class WDataTable : MonoBehaviour
    {
        ...
        public void UpdateData(IList<IList<object>> datas)
        {
            ...
            if (!m_init)
            {
                Debug.LogError("データテーブルが初期化されていません。");
                return;
            }
        ...
        private void UpdateScrollRectSize()
        {
            ...
            m_scrollbarRectTransform.anchoredPosition = new Vector2(0, 0);
        ...
        private bool CheckConfig()
        {
            ...
            {
                Debug.LogError("need itemHeight < tebleHeight");
                return false;
            }
        ...
        private bool CheckInputData(IList<IList<object>> datas, ICollection<WColumnDef> columnDefs)
        {
            ...
            foreach (var column in columnDefs)
            {
                ...
                if (string.IsNullOrEmpty(defaultHeadPrefabName) && string.IsNullOrEmpty(column.elementPrefabName))
                {
                    Debug.Log("if defaultHeadPrefabName is empty, columnDefs need contain headPrefabName");
                    return false;
                }

                if (string.IsNullOrEmpty(defaultElementPrefabName) && string.IsNullOrEmpty(column.elementPrefabName))
                {
                    Debug.Log("if defaultElementPrefabName is empty, columnDefs need contain elementPrefabName");
                    return false;
                }
            }
